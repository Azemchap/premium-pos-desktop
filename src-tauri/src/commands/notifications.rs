...
#[command]
pub async fn check_pending_invoices(
    pool: State<'_, SqlitePool>,
) -> Result<i32, String> {
    let pool_ref = pool.inner();
    ...
    for row in pending_invoices {
        ...
        if existing.is_none() {
            ...
            let message = format!(
                ...
                po_number, supplier_info, payment_status.to_lowercase(), total_amount
            );

            let severity = if payment_status == "Unpaid" { "error" } else { "warning" };
...
#[command]
pub async fn check_outstanding_debts(
    pool: State<'_, SqlitePool>,
) -> Result<i32, String> {
    let pool_ref = pool.inner();
    ...
    for row in outstanding_debts {
        ...
        if existing.is_none() {
            ...
            let message = format!(
                ...
                sale_number, customer_info, payment_status.to_lowercase(), total_amount
            );

            let severity = if payment_status == "Pending" { "error" } else { "warning" };
...
pub async fn refresh_notifications(
    ...
    let low_stock_count = check_low_stock_alerts(State::from(pool_ref.clone())).await?;
    let invoice_count = check_pending_invoices(State::from(pool_ref.clone())).await?;
    let debt_count = check_outstanding_debts(State::from(pool_ref.clone())).await?;
