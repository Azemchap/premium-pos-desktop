...
#[command]
pub async fn get_all_variant_types(
    pool: State<'_, SqlitePool>,
) -> Result<Vec<VariantType>, String> {
    ...
    .fetch_all(pool_ref)
    .await
    .map_err(|e| format!("Failed to fetch variant types: {}", e))?;

    rows.iter().map(build_variant_type_from_row).collect()
}
...
pub async fn get_variant_type(pool: State<'_, SqlitePool>, id: i64) -> Result<VariantType, String> {
    ...
    .fetch_one(pool_ref)
    .await
    .map_err(|e| format!("Variant type not found: {}", e))?;

    build_variant_type_from_row(&row)
}
...
pub async fn create_variant_type(
    ...
    .execute(pool_ref)
    .await
    .map_err(|e| format!("Failed to create variant type: {}", e))?;
...
pub async fn update_variant_type(
    ...
    .execute(pool_ref)
    .await
    .map_err(|e| format!("Failed to update variant type: {}", e))?;
...
pub async fn delete_variant_type(pool: State<'_, SqlitePool>, id: i64) -> Result<(), String> {
    ...
    sqlx::query("DELETE FROM variant_types WHERE id = ?1")
        ...
        .execute(pool_ref)
        .await
        .map_err(|e| format!("Failed to delete variant type: {}", e))?;
...
pub async fn get_variant_values_by_type(
    ...
    .fetch_all(pool_ref)
    .await
    .map_err(|e| format!("Failed to fetch variant values: {}", e))?;

    rows.iter().map(build_variant_value_from_row).collect()
}

#[command]
pub async fn get_all_variant_values(
    pool: State<'_, SqlitePool>,
) -> Result<Vec<VariantValue>, String> {
    let pool_ref = pool.inner();
    ...
    .fetch_all(pool_ref)
    .await
    .map_err(|e| format!("Failed to fetch all variant values: {}", e))?;
...
pub async fn get_variant_value(
    ...
    .fetch_one(pool_ref)
    .await
    .map_err(|e| format!("Variant value not found: {}", e))?;

    build_variant_value_from_row(&row)
}
...
pub async fn create_variant_value(
    ...
    .execute(pool_ref)
    .await
    .map_err(|e| format!("Failed to create variant value: {}", e))?;
...
pub async fn update_variant_value(
    ...
    .execute(pool_ref)
    .await
    .map_err(|e| format!("Failed to update variant value: {}", e))?;
...
pub async fn delete_variant_value(pool: State<'_, SqlitePool>, id: i64) -> Result<(), String> {
    ...
    sqlx::query("DELETE FROM variant_values WHERE id = ?1")
        ...
        .execute(pool_ref)
        .await
        .map_err(|e| format!("Failed to delete variant value: {}", e))?;
...
pub async fn get_product_variants(
    ...
    for variant_row in &variant_rows {
        ...
        let variant_values = fetch_variant_values_for_variant(pool_ref, variant_id).await?;

        // Fetch inventory
        let inventory = fetch_variant_inventory_optional(pool_ref, variant_id).await?;
...
pub async fn create_product_variant(
    ...
    for value_id in &request.variant_value_ids {
        ...
        .execute(pool_ref)
        .await
        .map_err(|e| format!("Failed to link variant value: {}", e))?;
    }
    ...
    .execute(pool_ref)
    .await
    .map_err(|e| format!("Failed to create variant inventory: {}", e))?;

    // Fetch the complete variant directly
    fetch_single_product_variant(pool_ref, variant_id).await
}
...
pub async fn get_product_variant(
    ...
    .fetch_one(pool_ref)
    .await
    .map_err(|e| format!("Product variant not found: {}", e))?;

    build_product_variant_from_row(&row)
}
...
pub async fn update_variant_inventory(
    ...
    .execute(pool_ref)
    .await
    .map_err(|e| format!("Failed to update variant inventory: {}", e))?;
...
pub async fn get_variant_inventory(
    ...
    .fetch_one(pool_ref)
    .await
    .map_err(|e| format!("Variant inventory not found: {}", e))?;

    build_variant_inventory_from_row(&row)
}

// ==================== HELPER FUNCTIONS ====================

/// Build VariantType from database row
fn build_variant_type_from_row(row: &sqlx::sqlite::SqliteRow) -> Result<VariantType, String> {
    Ok(VariantType {
        id: row.get("id"),
        name: row.get("name"),
...
/// Build VariantValue from database row
fn build_variant_value_from_row(row: &sqlx::sqlite::SqliteRow) -> Result<VariantValue, String> {
    Ok(VariantValue {
        id: row.get("id"),
...
/// Build ProductVariant from database row
fn build_product_variant_from_row(
...
/// Build VariantInventory from database row
fn build_variant_inventory_from_row(
    row: &sqlx::sqlite::SqliteRow,
) -> Result<VariantInventory, String> {
    Ok(VariantInventory {
        id: row.get("id"),
...
/// Fetch variant values for a specific variant
async fn fetch_variant_values_for_variant(
    pool: &SqlitePool,
    variant_id: i64,
) -> Result<Vec<VariantValue>, String> {
    ...
    value_rows.iter().map(build_variant_value_from_row).collect()
}

/// Fetch variant inventory (optional - returns None if not found)
async fn fetch_variant_inventory_optional(
    pool: &SqlitePool,
    variant_id: i64,
) -> Result<Option<VariantInventory>, String> {
    ...
    .fetch_optional(pool)
    .await
    .map_err(|e| format!("Failed to fetch variant inventory: {}", e))?;

    match inventory_row {
        Some(row) => Ok(Some(build_variant_inventory_from_row(&row)?)),
        None => Ok(None),
    }
}

/// Fetch a single product variant with all its details
async fn fetch_single_product_variant(
    pool: &SqlitePool,
    variant_id: i64,
) -> Result<ProductVariantWithValues, String> {
    ...
    let variant_values = fetch_variant_values_for_variant(pool, variant_id).await?;
    let inventory = fetch_variant_inventory_optional(pool, variant_id).await?;

    Ok(ProductVariantWithValues {
        id: variant_row.get("id"),
        product_id: variant_row.get("product_id"),
        sku: variant_row.get("sku"),
        barcode: variant_row.try_get("barcode").ok().flatten(),
